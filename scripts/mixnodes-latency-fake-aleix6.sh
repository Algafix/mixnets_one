echo "value scenario maxTime" > data/histrLatencyFakeMixNodes6.data 
...
file=reports_sim/LatencyFakeMixNodes6/$a/*$mix\_MessageDetailedReport.txt
#ls -la $file 2>/dev/null
IFS="
"
for line in `cat $file 2>/dev/null|grep -v Mess|grep -v sim 2>/dev/null`
do
echo $line $mix >> data/histrLatencyFakeMixNodes6.data
done
done
done
